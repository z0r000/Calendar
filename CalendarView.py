...
class CalendarView:
	...
	def setup(self, y, m):
		...
		Tasks=[]
		bookedDays=[]
		with open("Tasks.txt", 'r') as appFile:
			reader = csv.reader(appFile)
			for row in reader:
				# removes empty list from loop
				if len(row) > 0:
					#Check for tasks made by this user
					if row[0] in self.values:
						#Parse the date into day,month,year
						dayInt,monthInt,yearInt = row[2].split("/")
						Tasks.append((int(dayInt),int(monthInt),int(yearInt)))
		...
		for w, week in enumerate(self.cal.monthdayscalendar(y, m), 2):
			for d, day in enumerate(week):
				if day:
					...
					color = self.COLOR_OF_APP_DAY_BUTTONS if (day, m, y) in Tasks else self.COLOR_OF_DAY_BUTTONS
